...
public unsafe class DDSProcessor : ContentProcessor<DDSContent>
{
    ...
    public override void Process(DDSContent item, string name, string outDir)
    {
        ...
        {
            _gl.TexImage2D(TextureTarget.Texture2D, 0, isCompressed ? InternalFormat.Rgba8 : iFmt, (uint) result.Width,
                (uint) result.Height, 0, PixelFormat.Rgba, PixelType.UnsignedByte, pData);
        }
        
        _gl.GenerateMipmap(TextureTarget.Texture2D);

        int numMips = (int) float.Floor(float.Log2(int.Max(result.Width, result.Height))) + 1;
        ...
        uint flags = 0x1 | 0x2 | 0x4 | 0x8 | 0x1000 | 0x20000;
        if (isCompressed)
            flags |= 0x80000;
        ...
        writer.Write(0x1000 | 0x8 | 0x400000); // dwCaps
